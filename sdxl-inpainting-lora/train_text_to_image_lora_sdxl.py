...
import os
def save_tensor_images_raw(batch, save_dir='visualization'):
    os.makedirs(save_dir, exist_ok=True)
    saved_images = {}
    
    # For images with range [-1, 1], convert to [0, 1]
    def normalize_neg1_to_1(tensor):
        return (tensor + 1.0) / 2.0
    
    # Save original image (range [-1, 1] -> [0, 1])
    original_img = normalize_neg1_to_1(batch['pixel_values'])
    original_path = os.path.join(save_dir, 'original_image.png')
    vutils.save_image(original_img, original_path)
    saved_images['original'] = original_path
    
    # Save conditioning image (range [-1, 1] -> [0, 1])
    conditioning_img = normalize_neg1_to_1(batch['conditioning_pixel_values'])
    conditioning_path = os.path.join(save_dir, 'conditioning_image.png')
    vutils.save_image(conditioning_img, conditioning_path)
    saved_images['conditioning'] = conditioning_path
    
    # Save mask (already in range [0, 1], no need to normalize)
    mask_path = os.path.join(save_dir, 'mask.png')
    vutils.save_image(batch['mask_values'], mask_path)
    saved_images['mask'] = mask_path
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            tracker.log(
                {
                    phase_name: [
                        wandb.Image(image, caption=f"{i}: {args.validation_prompts}") for i, image in enumerate(images)
                    ]
                }
            )
    return images
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        #         )
        # # pipeline.load_lora_weights("/home/ec2-user/ebs-px/SD-diffusers/diffusers/examples/sdxl-inpaint-lora/data/train_csr/SD-M6/MODEL_OUT_512/sdxl_inpainting_lora/checkpoint-20000/")
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                # # Usage:
                # saved_paths = save_tensor_images_raw(batch)
                # print("Images saved at:", saved_paths)
                ...
                # # convert masked_image to latent space
                # masked_latents = vae.encode(batch["conditioning_pixel_values"].to(weight_dtype)).latent_dist.sample()
